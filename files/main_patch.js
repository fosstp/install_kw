...
function OnUpdateConfig(strBase, strOld, strNew)
{
	try
	{
		...
		var xmlNew = _LoadXML(urlNew, true);
		if (xmlNew == null)
			return;

		var xmlOld = _LoadXML(urlOld, true);
		if (xmlOld != null)
		{
			...
			// save xml
			_SaveXML(xmlOld, urlOld, true);
		}
		else
		{
			_SaveXML(xmlNew, urlOld, true);
		}
		if (_LoadXML(xmlNew, urlOld, true) != null)
			_DelFile(urlNew);
	}
	catch (e)
	{
...
function OnConfigInitialize(bInitialize)
{
	try
	{
		...
		g_oConfig.SetData("SYSTEM_DIRECTORY", "file:///" + dirEIC.replace(/\\/g, "/"));
		...
		var xmlTmp = _LoadXML(_G("SYSTEM_AP_DIRECTORY") + "xml/config/config.xml", true);
		if (xmlTmp == null)
...
function SyncAdbook(nCount)
{
	try
	{
		var bAutoSync = true;
		// 檢核差異值
		var nSync = _GN("SYSTEM_ADBOOK_SYNC_DIFF", 2000);
		if (nCount >= nSync)
		{
			...
			if (nSync > 0) // 同步通訊錄
			{
				window.open(_Url(sSyncUrl + "&SYNC_TYPE=10"), "_blank", "width=500, height=180, toolbar=no, menubar=no, scrollbars=no, status=no, titlebar=no, location=no, resizable=yes");
				bAutoSync = false;
			}
			else // 同步檔案
			{
				var xml = _LoadXML(_G("SYSTEM_DIRECTORY") + "version.xml", true);
				var sSpSub = _TC(_G("FORMBINDER_SP_VERSION"), "_", _G("FORMBINDER_SP_SUB_VERSION"));
				if (xml != null && (_XAN(xml.documentElement, "release") != Math.abs(nSync) || _XA(xml.documentElement, "sp_sub") != sSpSub))
					// 不同版本，計算HASH & 同步
				{
					(function (xnDir, sDir)
					{
						var xnList = xnDir.selectNodes("DIR");
						for (var n = 0; n < xnList.length; n++)
						{
							arguments.callee(xnList[n], sDir + xnList[n].getAttribute("name") + "\\");
						}
						xnList = xnDir.selectNodes("FILE");
						for (var m = xnList.length - 1; m >= 0; m--)
						{
							var xnFile = xnList[m];
							var sHash = _HashFile(sDir + xnFile.getAttribute("name"));
							if (sHash)
								xnFile.text = sHash;
							else
								xnDir.removeChild(xnFile);
						}
					})(xml.documentElement, _Dir(_G("SYSTEM_DIRECTORY")));
					_SaveXML(xml, _G("SYSTEM_DIRECTORY") + "version.xml", true);
					xml = null;
				}
				if (xml == null) // 同步檔案 & 停用表單同步
				{
					document.getElementById("elmSyncFrame").src = _Url(sSyncUrl + "&SYNC_TYPE=" + nSync);
					_SetG("SYSTEM_SYNC_RESOURCE", "N");
				}
			}
		}
...
function OnLoadXMLFromServer()
{
	...
	if (g_xmlDoc.documentElement != null)
	{
		try
		{
			...
			var xmlConfig = _LoadXML(urlSysConfig, true);
...
function _FormbinderInstalled()
{
	try
	{
		if (_LoadXML(_G("SYSTEM_OFFLINE_DIRECTORY") + "xml/config/config.xml", true) != null)
			return true;
		...
		if (_IsLocalFileExist(pathUserConfig) == false)
		{
			if (bAutoReg)
			{
				try
				{
					...
					var xmlSys = _LoadXML(urlSysConfig, true);
...
function _ModuleInstalled(urlSysSererConfig)
{
	try
	{
		...
		var pathSysConfig = UrlToPath(urlSysConfig, true);
...
function _SetModuleConfigData()
{
	...
	try
	{
		...
		var oXmlDocTmp = _LoadXML(urlSysConfig, true);
...
function _SyncResourceWithServer()
{
	try
	{
		...
		document.getElementById("elmSyncFrame").src = sSyncUrl;
		window["elmSyncFrame"].document.bgColor = _GetCssText(".cssFootBG", "backgroundColor"); //"#558AAB"; 
	}
	catch (e) { }
}
